...
export default function WalletPage({
    ...
    return (
        <>
            ...
            <div className="sticky top-0 z-30 bg-primary dark:bg-primary/95 backdrop-blur-md text-white py-3.5 px-4 flex md:hidden items-center justify-between shadow-sm">
                <div className="flex items-center gap-3 min-w-0">
                    <Link
                        href={dashboard().url}
                        className="p-1 text-white/90 hover:text-white transition-colors cursor-pointer shrink-0"
                    >
            ...
            <div className="w-full max-w-4xl mx-auto px-3.5 sm:px-6 py-4 sm:py-6 space-y-4 sm:space-y-6 overflow-x-hidden">
                {/* ── HERO WALLET BALANCE CARD ── */}
                <div className="relative overflow-hidden rounded-2xl bg-white dark:bg-[#1e1e2d] p-4 sm:p-6 shadow-[0_10px_25px_-5px_rgba(98,54,255,0.08),0_4px_10px_-2px_rgba(0,0,0,0.04)] border border-gray-100/80 dark:border-gray-800">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div className="space-y-1 sm:space-y-2">
                            <div className="flex items-center justify-between sm:justify-start gap-2">
                                <div
                                    className="flex items-center gap-1.5 text-gray-500 dark:text-gray-400 text-xs font-semibold uppercase tracking-wider cursor-pointer select-none"
                                    onClick={toggleBalance}
                                >
                                    <WalletIcon className="w-4 h-4 text-primary" />
                                    <span>Available Balance</span>
                                    {showBalance ? (
                                        <EyeOff className="w-3.5 h-3.5 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300" />
                                    ) : (
                                        <Eye className="w-3.5 h-3.5 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300" />
                                    )}
                                </div>

                                <span className="inline-flex sm:hidden items-center gap-1.5 px-2 py-0.5 rounded-full text-[10.5px] font-bold bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20">
                            ...
                            <div className="flex items-baseline gap-1">
                                <span className="text-2xl sm:text-3xl md:text-4xl font-extrabold tracking-tight text-gray-900 dark:text-white">
                                    {showBalance
                        ...
                        {/* Quick Action Buttons (2-column responsive grid on mobile, flex on desktop) */}
                        <div className="grid grid-cols-2 gap-2 sm:flex sm:items-center sm:gap-3 pt-1 sm:pt-0">
                            <button
                                onClick={() => setIsAddMoneyOpen(true)}
                                className="w-full sm:w-auto inline-flex items-center justify-center gap-1.5 px-3.5 py-2.5 sm:px-5 sm:py-2.5 rounded-xl bg-primary hover:bg-primary/90 text-primary-foreground font-bold text-xs sm:text-sm shadow-md shadow-primary/20 transition-all active:scale-98 cursor-pointer"
                            >
                            ...
                            {activeVirtualAccount ? (
                                <button
                                    ...
                                    className="w-full sm:w-auto inline-flex items-center justify-center gap-1.5 px-3 py-2.5 sm:px-4 sm:py-2.5 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-zinc-800 dark:hover:bg-zinc-700 text-slate-800 dark:text-slate-200 font-bold text-xs sm:text-sm border border-slate-200/80 dark:border-zinc-700 transition-all active:scale-98 cursor-pointer"
                                >
                                    {copiedAccount ? <Check className="w-4 h-4 text-emerald-500 shrink-0" /> : <Copy className="w-4 h-4 text-slate-400 shrink-0" />}
                                    <span className="truncate">{copiedAccount ? 'Copied' : 'Copy Acct'}</span>
                                </button>
                            ) : (
                                <button
                                ...
                                >
                                    ...
                                    <span className="truncate">Get Account</span>
                                </button>
                            )}
                        </div>
                    </div>
                ...
                <div className="bg-white dark:bg-[#1e1e2d] rounded-2xl p-4 sm:p-6 border border-gray-100/80 dark:border-gray-800 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] space-y-4">
                    <div className="flex items-center justify-between pb-3 border-b border-gray-100 dark:border-gray-800 gap-2">
                        <div className="flex items-center gap-2.5 min-w-0">
                            ...
                            <div className="min-w-0">
                                <h2 className="font-bold text-slate-900 dark:text-white text-sm sm:text-base truncate">
                        ...
                        <span className={cn(
                            "shrink-0 inline-flex items-center gap-1 px-2 sm:px-2.5 py-1 rounded-md text-[10.5px] sm:text-[11px] font-bold uppercase tracking-wider border",
                            ...
                            <span>
                                {store?.customer_funding_fee_type === 'flat' && Number(store?.customer_funding_fee_amount) > 0
                                    ? `₦${Number(store.customer_funding_fee_amount).toLocaleString()} Fee`
                                    : store?.customer_funding_fee_type === 'percentage' && Number(store?.customer_funding_fee_amount) > 0
                                    ? `${store.customer_funding_fee_amount}% Fee`
                                    : '0% Fee (Free)'}
                            </span>
                        </span>
                    </div>
                    ...
                    {activeVirtualAccount ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4 pt-1">
                            {/* Bank Details Box */}
                            <div className="p-3.5 sm:p-4 rounded-xl bg-slate-50/80 dark:bg-zinc-900/60 border border-slate-200/70 dark:border-zinc-800 space-y-2.5 sm:space-y-3">
                                <div className="flex items-center justify-between">
                                    ...
                                    <div className="flex items-center gap-1.5">
                                        <img src={palmpayIcon} alt="PalmPay" className="w-4 h-4 rounded-full object-contain shrink-0" />
                                ...
                                <div className="flex items-center justify-between pt-1">
                                    <div className="space-y-0.5">
                                        ...
                                        <div className="text-lg sm:text-2xl font-mono font-extrabold text-slate-900 dark:text-white tracking-wider">
                                    ...
                                    <button
                                        onClick={() => handleCopyAccount(activeVirtualAccount.account_number)}
                                        className={cn(
                                            'px-3 py-1.5 sm:py-2 rounded-lg font-bold text-xs flex items-center gap-1.5 transition-all cursor-pointer shadow-xs',
                                            copiedAccount
                            ...
                            {/* How It Works Note */}
                            <div className="p-3.5 sm:p-4 rounded-xl bg-amber-500/5 dark:bg-amber-500/10 border border-amber-500/20 flex flex-col justify-between space-y-2.5 sm:space-y-3">
                                ...
                                <div className="text-[11px] text-slate-500 dark:text-slate-400 flex items-center gap-1.5 pt-1 border-t border-amber-500/10">
                                    <Clock className="w-3.5 h-3.5 text-primary shrink-0" />
                                    <span>24/7 Automated processing • Zero manual confirmation</span>
                                </div>
                            </div>
                        </div>
                    ) : (
                ...
                <div className="bg-white dark:bg-[#1e1e2d] rounded-2xl p-4 sm:p-6 border border-gray-100/80 dark:border-gray-800 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] space-y-3 sm:space-y-4">
                    <div className="flex items-center justify-between pb-3 border-b border-gray-100 dark:border-gray-800">
                        <div className="flex items-center gap-2.5">
                            <div className="w-9 h-9 rounded-xl bg-slate-100 dark:bg-zinc-800 text-slate-700 dark:text-slate-300 flex items-center justify-center shrink-0">
